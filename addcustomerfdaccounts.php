...
$sql = "update accounts set customer_id='$_GET[customeracid]', acc_status='$_POST[accountstatus]', primary_acc='$_POST[primaryaccount]', acc_type='$_POST[accounttype]',acc_balance='$_POST[accountbalance]',unclear_bal='$_POST[unclearbalance]',interest='$_POST[interest]' where acc_no ='$_GET[editid]'";
		...
		{
			$sql = "insert into accounts(acc_no,customer_id,acc_status,acc_date,fd_id,acc_balance,unclear_bal,interest) VALUES('$_POST[accno]','$_SESSION[customer_id]','Active','$dt','$_POST[accounttype]','$_POST[amtloanamount]','$_POST[unclearbalance]','$_POST[interest]') ";
			$qsql = mysqli_query($con,$sql);
			if(!$qsql)
			{
				echo mysqli_error($con);
			...
			if(mysqli_affected_rows($con) == 1)
			{
					//Deduct funds from Transaction after creating fd account							
					$sql = "insert into transaction( registered_payee_id,  to_acc_no, amount, comission, particulars, transaction_type, trans_date_time, approve_date_time, payment_status) VALUES ('$_POST[regpayeeid]','$_POST[account]','$_POST[amount]','0','$_POST[particulars]','Debit','$dttim','$dttim','Active') ";
					$qsql = mysqli_query($con,$sql);
					...
					$qsql = mysqli_query($con,$sql);
					echo "<script>alert(Fixed deposit account created successfully..');</script>";
					echo "<script>window.location='viewfdtransaction.php';</script>";
			}
			//#################################################################################//
  ...
  <script type="application/javascript">
	function javascriptvalidation()
	{
		...
		if(!document.frmaccounts.amtloanamount.value.match(numericfloatExpression))
			{
				document.getElementById("jsamtloanamount").innerHTML ="<font color='red'><strong>Investment amount should be floating point numbers.</strong></font>";
				validateform=1;
			}  
