...
if($_SESSION['randomno'] == $_POST['randomno'])
{
	if(isset($_POST['submit'])) 
	{
		...
		else
		{
			...
			if($_POST[deposittype] == "Cash")
			{
				$sql = "insert into transaction(to_acc_no,amount,comission,particulars,transaction_type,trans_date_time,approve_date_time,payment_status) VALUES('$_POST[account]','$_POST[amount]','$_POST[commission]','$_POST[particulars]','Credit','$dttim','$dttim','Active') ";
				$qsql = mysqli_query($con,$sql);
				if(!$qsql)
				{
					echo mysqli_error($con);
				...
				echo "<script>window.location='depositmoneyreceipt.php?receiptid=" . $insid ."';</script>";
			}
			
			if($_POST[deposittype] == "Cheque")
			{			
				$sql = "insert into transaction(to_acc_no,amount,comission,particulars,transaction_type,trans_date_time,approve_date_time,payment_status) VALUES('$_POST[account]','$_POST[amount]','$_POST[commission]','$_POST[particulars]','Cheque','$dttim','$dttim','Pending') ";
				$qsql = mysqli_query($con,$sql);
				if(!$qsql)
				{
					echo mysqli_error($con);
				...
				$insid= mysqli_insert_id($con);			
				$sql = "UPDATE accounts SET unclear_bal= unclear_bal +  $_POST[amount]  WHERE acc_no='$_POST[account]'";
				$qsql = mysqli_query($con,$sql);
				if(!$qsql)
				{
					echo mysqli_error($con);
...
?>
	...
	function javascriptvalidation()
	{
		...
		document.getElementById("jsdeposittype").innerHTML ="";
		...
		if(document.frmtransaction.deposittype.value=="")
		{
			document.getElementById("jsdeposittype").innerHTML ="<font color='red'><strong>Kindly select deposit type.</strong></font>";
			validateform=1;
		}
